...
void ColoredProgressBar::paintColor(QPainter& painter, QColor color, int beginPosition, int endPosition)
{
    painter.setPen(color);
    painter.setBrush(QBrush(color));
    painter.drawRect(beginPosition,0,endPosition,height());
}
...
void ColoredProgressBar::paintEvent(QPaintEvent*)
{
    ...
    {
        paintColor(painter, Qt::cyan, pos0, currentPosition);
    }
    else if(currentValue > 20 && currentValue <= 40)
    {
        paintColor(painter, Qt::cyan, pos0, pos20);
        paintColor(painter, Qt::green, pos20, currentPosition);
    }
    else if(currentValue > 40 && currentValue <= 60)
    {
        paintColor(painter, Qt::cyan, pos0, pos20);
        paintColor(painter, Qt::green, pos20, pos40);
